...
const betHistory = require('../model/wheeloffortune');
...
router.post('/getGameResult', common.userVerify, function(req, res) {
	...
	common.findUserBalance(userId, currency, function(balance){
		if(balance >= bet_amount) {
			userHash.findOne({user_id:userId,game:"fortune"}).exec(async function(err, resp) {
				if(err) {
					return res.json({success:0,msg:"Please try again"});
				}
				if(resp) {
					var result= game.ringofFortune(resp.server_seed,resp.client_seed,resp.nonce);
					var nounce= parseInt(resp.nonce);
					var serverSeed = resp.server_seed;
				    var clientSeed  = resp.client_seed;
				} else {
					var user_has = await game.generateUserHash(userId,"fortune");
					var result= game.ringofFortune(user_has.client_seed, user_has.server_seed,user_has.nonce);
					var serverSeed = user_has.server_seed;
					var clientSeed = user_has.client_seed;
					var nounce 	   = parseInt(user_has.nonce); 
				}
				userHash.updateOne({user_id:userId,game:"fortune"}, {"$set": {nonce: nounce+1}}).exec(function(err1, res1){
...
router.post('/saveBetHistory', common.userVerify, async function(req, res){
	try{
		var initate_bet = req.body.bet_amount;var currency = req.body.currency;var game_result= req.body.game_result;
		var pay_out = req.body.pay_out;var userId = req.userId;var clientSeed = req.body.clientSeed;
		var serverSeed = req.body.serverSeed;var nonce = req.body.nonce;var result = req.body.result;
		var bet_amount = initate_bet.reduce((total, item) => total + item.amount, 0);
		var getres = initate_bet.find(item => item.color == game_result.color);

		const filtered = initate_bet.filter(item => item.amount != 0);const filteredLength = filtered.length;

		const totalAmount = initate_bet.reduce((sum, item) => {if (item.amount !== 0) { return sum + parseFloat(item.amount); }return sum; }, 0);

		common.findUserBalance(userId, currency, async function(userBalance){
			if(totalAmount <= userBalance) {
				if(getres.amount!==0){
					var cal=(filteredLength >= pay_out) ? filteredLength/pay_out : pay_out/filteredLength;
					var checkPayout=cal;var amount;

					if((getres.amount*pay_out)>totalAmount){ amount = (getres.amount*pay_out)-totalAmount ;
					}else{ amount = totalAmount-(getres.amount*pay_out);}

					var updated_balance=userBalance+amount;
					var balance = await common.updateUserBalance(userId, currency, updated_balance);
					if(amount>=totalAmount){ var status = "winner";
					}else{ var status = "loser"; }
				}else{
					var cal=(filteredLength >= pay_out) ? filteredLength/pay_out : pay_out/filteredLength;
					var checkPayout=cal;
					var amount = totalAmount-(getres.amount*pay_out);
					var updated_balance = userBalance - amount;
					var balance = await common.updateUserBalance(userId, currency, updated_balance);
					var status = "loser";
				}
				...
				if(status == "loser"){
	                common.findAdminBal(currency, async function(AdminBal){
		                ...
		                /*let profitJson = {user_id: mongoose.mongo.ObjectId(userId), currency: currency, amount: amount, type: "wheel-of-fortune"}
				...
				var res_amt;
				if(obj.status=="winner"){ res_amt=obj.win_lose_amt;
				}else{ res_amt=0; }
				...
				betHistory.create(obj, function(err, resData) {
					...
					if(resData) {
						betHistory.find({user_id:mongoose.mongo.ObjectId(userId)}).sort({_id : -1}).exec(function(err1, res1){
							users.updateOne({"_id":userId,"gamecount.name":"fortune"},{$inc
							:{"gamecount.$.total_count":1}},(ers,doc)=>{
	    						if(doc){
	    							common.wagecountUpdate(currency,userId,totalAmount,resData.win_lose_amt,status,"fortune",function(data) {})
    								common.gamecountUpdate(userId,"fortune",status,currency,function(data) {});
    								common.sendGameResult(resData);
    								res.json({success:1,status:obj.status ,balance : updated_balance, currency: currency, history : res1,payOut:obj.payout,resAmt:res_amt})
	    						}else{
...
router.post('/getBetHistory', common.userVerify, function(req, res){
	...
	betHistory.find({user_id:mongoose.mongo.ObjectId(userId),game:"fortune"}).sort({_id : -1}).limit(50).exec(function(err1, res1){
...
router.post('/getAllBetHistory', function (req, res) {
    betHistory.find({game:"fortune"}).sort({_id : -1}).limit(50).exec(function(err1, res1){
        if(res1.length !== 0){
...
router.post('/getDetails',common.userVerify, function (req, res) {
	...
	betHistory.findOne({'_id':_id,'user_id':userId,'game':game}).sort({_id : -1}).limit(50).exec(function(err,resp){
		if(err){
            ...
            users.findOne({'_id':userId},{username:1,_id:0}).exec(function(Errs,Resp) {
            	...
            	if(Resp){
            		var data={"bet_id":resp._id,"username":Resp.username,"created_at":resp.created_at,"bet_amount":resp.bet_amount,"payout":resp.payout,"pro_amt":resp.win_lose_amt,"result":resp.bet_result,"serverSeed":val,"clientSeed":fss,"nounce":nounce,"status":resp.status,"risk":resp.risk,"segment":resp.segment,
            			"initial_bet":resp.initial_bet,"game_result":resp.game_result,'seedstatus':resp.seed_status,"server":serverSeed1,"client":clientSeed2}
...
router.post('/generateHash',common.userVerify, function (req, res) {
    ...
    userHash.findOne({user_id:userId,game:"fortune"}).exec(function(err, resp) {
        if(err) {
            return res.json({success:0,msg:"Please try again"});
        }   
        if(resp){
            userHash.updateOne({user_id:userId,game:"fortune"},{"$set":{new_client_seed:newCli}}).exec(function(errs,resData) { 
                if(errs){
...
router.post('/changeHash',common.userVerify, function (req, res) {
    ...
    userHash.findOne({user_id:userId,game:"fortune"}).exec(function(err, resp) {
        ...
        if(resp){
            userHash.updateOne({user_id:userId,game:"fortune"},{"$set":{client_seed:clientSeed, server_seed:serverSeed, nonce:nounce, new_server_seed:newSer, new_client_seed:newCli}}).exec(function(errs,resData) {
                if(errs){
                    return res.json({success:0,msg:"Something went wrong"});
                }
                if(resData){
                    betHistory.updateMany({user_id:userId,server_seed:oldserverSeed,game:"fortune"},{$set:{seed_status:1}}).exec(function(err1,resp1){
			...
			// if(resp.user_favourites.length > 0){
				...
				if(tot =="added"){
					users.updateOne({'_id':userId},{'user_favourites':storeData}).exec(function(errs,resData){
						...
						if(resData){
							gamelist.updateOne({'game_name':'fortune'},{$inc:{'fav_count':1}}).exec(function(Err,Res){
								if(Err){
									return res.json({success:0,msg:"Something went wrong"});	
								}
								if(Res){
									return res.json({success:1,msg:"Added to Favourites",game:"fortune"});	
								}
							})
						}
					})
			...
			// if(resp.liked_game.length > 0){
				...
				if(tot =="added"){
					users.updateOne({'_id':userId},{'liked_game':storeData}).exec(function(errs,resData){
						...
						if(resData){
							gamelist.updateOne({'game_name':'fortune'},{$inc:{'like_count':1}}).exec(function(Err,Res){
								if(Err){
									return res.json({success:0,msg:"Something went wrong"});	
								}
								if(Res){
									return res.json({success:1,msg:"Added to Liked",game:"fortune"});	
								}
							})
						}
					})
...
router.post('/getminbetamount', common.userVerify, function(req, res) {
	...
	/*gamelist.findOne({game_name:'fortune'}).exec(function(err1, res1){
        if(res1){
           	res.json({success:1,min_bet:res1.min_bet, max_bet:res1.max_bet});
        } else {res.json({success:0});}
	})*/
	gamelist.findOne({game_name:'fortune'}, {Curr:{$elemMatch:{currency:userCurr}}}).exec(function(err,resData){
		if(resData){
