...
const betHistory = require('../model/plinko_bethistory');
...
router.post('/getGameResult', common.userVerify, function(req, res) {
	...
	common.findUserBalance(userId, currency, function(balance){
		if(balance >= bet_amount) {
			userHash.findOne({user_id:userId,game:"plinko"}).exec(async function(err, resp) {
				if(err) {
	...
	common.findUserBalance(userId, currency, async function(userBalance){ 
		if(bet_amount <= userBalance) {
			if(result >= 1) {
				...
				var amount = bet_amount - (result * bet_amount);
				var updated_balance = userBalance - amount;
				var balance = await common.updateUserBalance(userId, currency, updated_balance);
				var status = "loser";
			}
			...
			betHistory.create(obj, function(err, resData) {
				...
				if(resData) {
					betHistory.find({user_id:mongoose.mongo.ObjectId(userId),game:"plinko"}).sort({_id : -1}).exec(function(err1, res1){
						users.updateOne({"_id":userId,"gamecount.name":"plinko"},{$inc:
    					{"gamecount.$.total_count":1}},(ers,doc)=>{
    						if(doc){
								common.wagecountUpdate(currency,userId,bet_amount,resData.win_lose_amt,status,"plinko",function(data) {})
								common.gamecountUpdate(userId,"plinko",status,currency,function(data) {})
...
router.post('/getBetFullHistory', common.userVerify, function(req, res){
	var userId = req.userId;
	betHistory.find({user_id:mongoose.mongo.ObjectId(userId),game:"plinko"}).sort({_id : -1}).limit(50).exec(function(err1, res1){
		res.json({success:1, history : res1});
	})
});

router.post('/getAllBetHistory', function (req, res) {
    betHistory.find({game:"plinko"}).sort({_id : -1}).sort({_id : -1}).limit(50).exec(function(err1, res1){
        if(res1.length !== 0){
            ...
            res1.forEach((value) => {
                ...
                users.findOne({'_id':value.user_id},{username:1,_id:0}).exec(function(err,resp){
                    var obj={betid:value._id,player:resp.username,time:value.created_at,payout:value.payout,profit:value.win_lose_amt,status:value.status,win_lose_amt:value.win_lose_amt}
                        storeData.push(obj)
...
router.post('/getHash',common.userVerify, function (req, res) {
    ...
    userHash.findOne({user_id:userId,game:"plinko"}).exec(async function(err, resp) {
        ...
        if(resp){
        	...
        	var user_has = await game.generateUserHash(userId, "plinko");
...
router.post('/changeHash',common.userVerify, function (req, res) {
    ...
    userHash.findOne({user_id:userId,game:"plinko"}).exec(function(err, resp) {
        if(err) {
            return res.json({success:0,msg:"Please try again"});
        }   
        if(resp){
...
router.post('/generateHash',common.userVerify, function (req, res) {
    ...
    userHash.findOne({user_id:userId,game:"plinko"}).exec(function(err, resp) {
        ...
        if(resp){
            userHash.updateOne({user_id:userId,game:"plinko"},{"$set":{new_client_seed:newCli}}).exec(function(errs,resData) { 
                if(errs){
			...
			// if(resp.user_favourites.length){
				...
				if(tot =="added"){
					users.updateOne({'_id':mongoose.mongo.ObjectId(userId)},{'user_favourites':storeData}).exec(function(errs,resData){
						...
						if(resData){
							gamelist.updateOne({'game_name':'plinko'},{$inc:{'fav_count':1}}).exec(function(Err,Res){
								if(Err){
									return res.json({success:0,msg:"Something went wrong"});	
								}
								if(Res){
									return res.json({success:1,msg:"Added to Favourites",game:"plinko"});
								}
							})
						}
					})
			...
			// if(resp.liked_game.length > 0){
				...
				if(tot =="added"){
					users.updateOne({'_id':userId},{'liked_game':storeData}).exec(function(errs,resData){
						...
						if(resData){
							gamelist.updateOne({'game_name':'plinko'},{$inc:{'like_count':1}}).exec(function(Err,Res){
								if(Err){
									return res.json({success:0,msg:"Something went wrong"});	
								}
								if(Res){
									return res.json({success:1,msg:"Added to Liked",game:"plinko"});	
								}
							})
						}
					})
	...
	betHistory.findOne({'_id':_id,'user_id':userId,'game':game}).exec(function(err,resp){
		...
		if(resp){
            ...
            users.findOne({'_id':userId},{username:1,_id:0}).exec(function(Errs,Resp) {
            	...
            	if(Resp){
            		var data={"bet_id":resp._id,"username":Resp.username,"created_at":resp.created_at,"bet_amount":resp.bet_amount,"payout":resp.payout,"pro_amt":resp.win_lose_amt,"result":resp.bet_result,"status":resp.status}
...
router.post('/getgraphed',common.userVerify, function (req, res) {
	...
	async.parallel({
		first:function(callback){
			betHistory.aggregate([{'$match':{'user_id':mongoose.mongo.ObjectId(userid),
		...
		second:function(callback) {
			...
			,'win': {'$sum': { '$cond': [{ '$eq': ["$status", "winner"] }, 1, 0] }},'loss':{'$sum':
			 { '$cond': [{ '$eq': ["$status", "loser"] }, 1, 0] }}}}, {'$project': {'pofit': {
			'$subtract':['$profit','$wagered']},_id:0,win:1,loss:1,wagered:1}}]).exec(callback)
		}
...
router.post('/getminbetamount', common.userVerify, function(req, res) {
	...
	/*gamelist.findOne({game_name:'plinko'}).exec(function(err1, res1){
        if(res1){
           	res.json({success:1,min_bet:res1.min_bet, max_bet:res1.max_bet});
        } else {res.json({success:0});}
	})*/
	gamelist.findOne({game_name:'plinko'}, {Curr:{$elemMatch:{currency:userCurr}}}).exec(function(err,resData){
		if(resData){
